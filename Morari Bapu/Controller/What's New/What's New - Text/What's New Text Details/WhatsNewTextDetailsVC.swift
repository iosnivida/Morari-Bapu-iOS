...
class WhatsNewTextDetailsVC: UIViewController {
  ...
  var arrTextDetails = [String:JSON]()
  ...
  func getTextDetails(){
    ...
    let paramater = ["app_id":Utility.getDeviceID(),
     ...
     WebServices().CallGlobalAPI(url: WebService_Text_Details, headers: [:], parameters: paramater as NSDictionary, HttpMethod: "POST", ProgressView: true) { ( _ jsonResponce:JSON? , _ strErrorMessage:String) in
      ...
      if(jsonResponce?.error != nil) {
        ...
        if jsonResponce!["status"].stringValue == "true"{
            ...
            self.arrTextDetails = jsonResponce!["data"]["NewText"].dictionaryValue
            self.arrFavourite = jsonResponce!["MyFavourite"].arrayObject! as NSArray
          
            self.lblCreatedBy.text = self.arrTextDetails["created_name"]!.stringValue
            self.lblTitle.text = self.arrTextDetails["title"]!.stringValue
            self.lblDecription.text = self.arrTextDetails["description"]!.stringValue
          
            let predicate: NSPredicate = NSPredicate(format: "SELF contains[cd] %@", self.arrTextDetails["id"]!.stringValue)
  ...
  @IBAction func btnShare(_ sender: Any) {
    ...
    let share_Content = "Text \n\n\(arrTextDetails["created_name"]!.stringValue) \n\n\(arrTextDetails["title"]!.stringValue) \n\n\(arrTextDetails["description"]!.stringValue)  \n\nThis message has been sent via the Morari Bapu App.  You can download it too from this link : https://itunes.apple.com/tr/app/morari-bapu/id1050576066?mt=8"
  ...
  @IBAction func btnFavourite(_ sender: Any) {
    ...
    let paramater = ["app_id":Utility.getDeviceID(),
                   "favourite_for":"17",
                   "favourite_id":arrTextDetails["id"]!.stringValue]
...
extension WhatsNewTextDetailsVC: MenuNavigationDelegate{
  ...
  extension WhatsNewTextDetailsVC : InternetConnectionDelegate{
    ...
    @objc private func reachabilityChanged( notification: NSNotification )
    {
      ...
      if reachability.connection == .wifi || reachability.connection == .cellular {
        ...
        if let wd = UIApplication.shared.delegate?.window {
          ...
          if(vc is WhatsNewTextDetailsVC){
