...
class DailyKathaClipSliderVC: UIViewController {
    
    @IBOutlet weak var cvDailyKathaClip: UICollectionView!
    var arrVideo = [JSON]()
    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.main.async {
            ...
            self.lblTitle.text = "Daily Katha Clip"
        ...
        if arrVideo.count != 0{
            DispatchQueue.main.async {
                self.cvDailyKathaClip.reloadData()
                self.cvDailyKathaClip.scrollToItem(at: self.indexPosition, at: .centeredHorizontally, animated: false)
                
                Utility.collectionViewNoDataMessage(collectionView: self.cvDailyKathaClip, message: "", textColor: UIColor.white)
            }
        }
...
extension DailyKathaClipSliderVC: MenuNavigationDelegate{
...
/*
extension DailyKathaClipSliderVC : UITableViewDelegate, UITableViewDataSource{
        ...
        if indexPath.row == arrVideo.count - 1{
            if is_Api_Being_Called == false{
                if currentPageNo <  totalPageNo{
                    ...
                    currentPageNo += 1
                    self.getDailyKatha(pageNo: currentPageNo)
                }
            }
        }
    }
...
extension DailyKathaClipSliderVC : InternetConnectionDelegate{
...
extension DailyKathaClipSliderVC: UICollectionViewDelegateFlowLayout, UICollectionViewDelegate, UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "DailyKathaClipSliderCollectionViewCell", for: indexPath) as! DailyKathaClipSliderCollectionViewCell
        
            var dict = arrVideo[indexPath.row]
            ...
            cell.btnYoutube.addTarget(self, action: #selector(btnYoutube(_:)), for: UIControl.Event.touchUpInside)
            ...
            cell.lblDuration.text = "(Duration: \(dict["video_duration"].stringValue))"
            cell.lblDate.text = Utility.dateToString(dateStr: dict["date"].stringValue, strDateFormat: "dd-MM-yyyy")
            ...
            cell.imgVideo.kf.indicatorType = .activity
            cell.imgVideo.kf.setImage(with: URL(string: "\(BASE_URL_IMAGE)\(dict["video_image"].stringValue)"), placeholder: placeHolder, options: [.transition(ImageTransition.fade(1))])
